<template>
  <div class="app-container">
    <el-row class="filter-container" :gutter="10">
      <el-col :sm="3">
        <el-select v-model="listQuery.type" size="medium" placeholder="付款类型" clearable>
      ...
      <el-col :sm="6">
        <el-date-picker v-model="listQuery.date_range" type="daterange" align="right" size="medium" unlink-panels
          range-separator="-" start-placeholder="支付开始日期" end-placeholder="支付结束日期" :picker-options="pickerOptions"
    ...
    <div class="table-container">
      <el-table v-loading="listLoading" :data="list" border fit highlight-current-row size="mini" style="font-size: 14px;">
        ...
        <el-table-column label="付款类型" min-width="200" class="fixed-width">
          <template slot-scope="{row}">
            <span v-if="row.type == 1">充值</span>
            <span v-if="row.type == 2">购买商品</span>
          </template>
        </el-table-column>
