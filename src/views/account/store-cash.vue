<template>
  <div class="app-container">
    <el-row class="filter-container" :gutter="10">
      ...
      <el-col :sm="4">
        <el-input v-model="listQuery.nickname" size="medium" prefix-icon="el-icon-search" placeholder="输入用户姓名搜索"
          clearable @keyup.enter.native="handleFilter" />
      </el-col>
      <el-col :sm="3">
      ...
      <el-table v-loading="listLoading" :data="list" border fit highlight-current-row size="mini" style="font-size: 14px;">
        <el-table-column label="手机号码" width="150">
          <template slot-scope="{row}">
            <span class="link-type" @click="handleViewStore(row.store_id)">{{ row.mobile }}</span>
          </template>
        </el-table-column>

        <el-table-column label="店铺名" width="120">
          <template slot-scope="{row}">
            <span>{{ row.name }}</span>
          </template>
        </el-table-column>
  ...
  export default {
    ...
    methods: {
      ...
      getList() {
        ...
        storeCashIndex(this.listQuery).then(response => {
