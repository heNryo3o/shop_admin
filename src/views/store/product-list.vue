    ...
    <div class="table-container">
      <el-table v-loading="listLoading" :data="list" border fit highlight-current-row size="mini" style="font-size: 14px;">
        ...
        <el-table-column label="商品类型" width="150">
          <template slot-scope="{row}">
            <span>{{ row.is_online == 1 ? '线上' : '线下' }}</span>
          </template>
        </el-table-column>
        ...
        <el-table-column label="操作" fixed="right" min-width="260" class-name="small-padding fixed-width">
          <template slot-scope="{row}">
            ...
            <el-button v-if="row.is_dapai == 1" v-waves v-permission="['role/change-status']" size="mini" type="warning"
              @click="handleChangeDapai(row,2)">
              取消推荐
            </el-button>
            <el-button v-else v-waves v-permission="['role/change-status']" size="mini" type="success" @click="handleChangeDapai(row,1)">
              设为推荐
            </el-button>
          </template>
        </el-table-column>
