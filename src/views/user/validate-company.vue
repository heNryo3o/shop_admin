<template>
  <div class="app-container">
    <el-row class="filter-container" :gutter="10">
      ...
      <el-col :sm="3">
        <el-select v-model="listQuery.status" placeholder="认证状态" size="small" clearable>
    ...
    <div class="table-container">
      <el-table v-loading="listLoading" :data="list" border fit highlight-current-row size="mini" style="font-size: 14px;">
        ...
        <el-table-column label="社会信用代码" width="200">
          <template slot-scope="{row}">
            <span>{{ row.licence_num }}</span>
          </template>
        </el-table-column>
        ...
        <el-table-column label="操作" fixed="right" min-width="180" class-name="small-padding fixed-width">
          <template slot-scope="{row}">
            <el-button v-waves v-permission="['role/change-status']" size="mini" type="primary" @click="handlePreview(row)">
              查看
            </el-button>
            <el-button
              v-if="row.status == 1"
    ...
    <!-- 新增/编辑用户信息弹框 -->
    <el-dialog title="查看认证信息" width="700px" :visible.sync="dialogVisible">
      <div class="preview-container">
        <div class="licence-pic">
          <el-image :src="temp.licence_pic" fit="contain" />
        </div>
        <p>
          企业名称: {{ temp.company }}
        </p>
        <p>
          社会信用代码: {{ temp.licence_num }}
        </p>
      </div>
